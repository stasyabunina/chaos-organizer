...
export default class Modal {
    ...
    render() {
        ...
        modalWrapper.className = 'modal-wrapper fixed flex justify-center items-center w-full h-full bg-black bg-opacity-50 z-50';

        const modal = document.createElement('div');
        modal.className = 'modal-box flex flex-col justify-center items-center p-14';
        ...
        usernameLabel.className = 'input input-bordered flex items-center gap-2 mb-6';
        ...
        passwordLabel.className = 'input input-bordered flex items-center gap-2 mb-6';
    ...
    formValidator(data) {
        ...
        const validateUsername = (username) => {
            ...
            if (!username.match(/^[a-zA-Z0-9]{5,20}$/)) {
                return { success: false, message: 'Имя пользователя должно быть минимум 5 и максимум 20 символов, не иметь пробелов и символов.' };
            }
        ...
        const validatePassword = (password) => {
            ...
            if (!password.match((/^(?=.*?[0-9])(?=.*?[A-Za-z]).{8,32}$/))) {
                return { success: false, message: 'Пароль должен быть минимум 8 и максимум 32 символов, иметь как минимум 1 цифру и 1 букву.' };
            }
        ...
        if (!usernameValidation.success && !passwordValidation.success) {
            if (usernameValidation.message === passwordValidation.message) {
                ...
                return { success: false, message: `${usernameValidation.message}<br>${passwordValidation.message}` }
            }
