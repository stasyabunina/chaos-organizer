...
export default class Pinned {
    ...
    render() {
        ...
        pinned.className = 'pinned py-6 absolute w-full top-0 z-50 bg-neutral bg-opacity-85';
        ...
        if (this.data.type === 'text') {
            ...
            type.textContent = this.data.text.length > 65 ? `${this.data.text.slice(0, 65).trim()}...` : this.data.text;
            text.append(type);
        } else if (this.data.type === 'link') {
            const type = document.createElement('p');
            const formattedText = this.data.text.replace(/\n/g, '');
            const slicedText = formattedText.length > 65 ? `${formattedText.slice(0, 65).trim()}...` : formattedText;
            text.append(type);
            ...
            const type = document.createElement('span');
            type.textContent = this.data.file.length > 65 ? `${this.data.file.slice(0, 65).trim()}...` : this.data.file;
            text.append(type);

            if (this.data.type === 'image') {
                const imgWrapper = document.createElement('div');
                imgWrapper.className = 'w-[50px] h-[50px] mr-3 flex items-center justify-center overflow-hidden';
            ...
            } else if (this.data.type === 'file' || this.data.type === 'audio' || this.data.type === 'video') {
                const file = document.createElement('div');
                file.className = 'w-[50px] h-[50px] mr-3 flex items-center justify-center bg-content';
