...
export default class Message {
	...
	render(method = "append") {
		...
		if (this.data.text) {
			...
			if (this.data.type === "image") {
				...
				content.classList.add("flex");
				this.audioController = new AudioController(content, this.data.file);
				...
				this.videoController = new VideoController(
					content,
					this.data.file,
					this.fileLoaderRemove.bind(this)
				);
			}
		}
	...
	renderMoreBtn(message) {
		...
		if (this.data.author === "bot") {
			moreBtn.className =
				"active:scale-[0.9] active:origin-center duration-100";
		} else {
			moreBtn.className =
				"active:scale-[0.9] active:origin-center duration-100";
		}
	...
	renderDropdown() {
		...
		dropdown.className =
			"dropdown-content menu bg-neutral rounded-box z-[1] w-52 p-2";
		...
		if (this.data.type === "text" || this.data.type === "link") {
			...
			toDataURL(
				this.data.author === "bot"
					? this.data.file
					: `${config.baseUrl}/${getCookie("username")}/${this.data.file}`
			).then((dataUrl) => {
	...
	removeMessage() {
		loaderRender();
		axios
			.post(`${config.baseUrl}/delete-message`, {
				apiKey: getCookie("api_key"),
				id: this.data.id,
				fileName: this.data.author === "bot" ? undefined : this.data.file,
			})
			.then(() => {
				...
				if (state.messages.length > 10) {
					...
					state.messages = state.messages.filter(
						(message) => message.id !== this.data.id
					);
					loaderRemove();
				} else {
					axios
	...
	favoriteMessage() {
		axios
			...
			.then(() => {
				...
				this.favoriteBtn.addEventListener("animationend", () =>
					this.favoriteBtn.removeAttribute("style")
				);
			})
			.catch((error) => {
				showErrorToast(error.message);
	...
	pinMessage() {
		...
		if (prevPinned) {
			axios
				.post(`${config.baseUrl}/delete-pinned`, {
					apiKey: getCookie("api_key"),
				})
				.then(() => {
					prevPinned.remove();
