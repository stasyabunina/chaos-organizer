...
export default class Widget {
	...
	bindToDOM() {
		...
		this.messagesWrapper = this.element.querySelector(
			".main__messages-wrapper"
		);
		this.messagesScrollWrapper = this.element.querySelector(
	...
	checkPinned() {
		loaderRender();
		axios
			.post(`${config.baseUrl}/get-pinned`, { apiKey: getCookie("api_key") })
			.then((response) => {
	...
	getMessages() {
		...
		axios
			...
			.catch((error) => {
				showErrorToast(
					(error.message += ". Перезагрузите страницу и попробуйте снова.")
				);
			})
			.finally(() => {
				loaderRemove();
	...
	scrollHandler() {
		this.messagesScrollWrapper.addEventListener("scroll", () => {
			if (
				this.messagesScrollWrapper.scrollTop < 50 &&
				this.messagesScrollWrapper.scrollTop !== 0
			) {
				this.loadMoreItems();
			}
		});
	...
	loadMoreItems() {
		if (state.isLoading || !state.hasMoreMessages || state.messages.length < 10)
			return;
		state.isLoading = true;

		const lastChild = this.messagesList.firstChild;

		this.messagesScrollWrapper.insertAdjacentHTML(
			"afterbegin",
			`<div class="more-loading-wrapper min-h-[230px] flex justify-center items-center w-full"><span class="loading loading-spinner loading-lg"></span></div>`
		);
			...
			axios
				.post(
					...
					state.isSearch
						? {
								...
								offset: state.offset,
								type: state.activeCategory,
								search: this.searchForm.input.value.trim(),
							}
						: {
								apiKey: getCookie("api_key"),
								offset: state.offset,
				...
				.finally(() => {
					this.messagesScrollWrapper
						.querySelector(".more-loading-wrapper")
						.remove();
	...
	showEmoji() {
		...
		emojiPickerWrapper.className =
			"main__emoji-picker-wrapper absolute bottom-16 right-0";
	...
	sendMessage(e) {
		...
		axios
			.post(`${config.baseUrl}/new-message`, {
				apiKey: getCookie("api_key"),
			...
			.then((response) => {
				if (state.file) {
					...
					fileFormData.append("file", state.file);
					axios
						.post(
							`${config.baseUrl}/upload/${getCookie("api_key")}`,
							fileFormData
						)
						.then(() => {
							if (
								state.activeCategory === newMessageData.type ||
								!state.activeCategory
							) {
								if (
									!state.isSearch ||
									(state.isSearch &&
										this.searchInput.value.includes(newMessageData.file))
								) {
									const message = new Message(
										newMessageData,
										this.messagesList,
